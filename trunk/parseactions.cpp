...
void CreateNassiBlockEnd::DoEnd() const
{
    ...
    m_brick->SetNext((NassiBrick *)NULL);
...
{
    //wxMessageDialog dlg(NULL, _T("close begin"), _T("test"));
    //dlg.ShowModal();
    ...
    if ( child && child->IsBlock() )
    {
        ...
        block->SetChild((NassiBrick *)NULL);
        //block->SetParent((NassiBrick *)NULL);
        block->SetPrevious((NassiBrick *)NULL);
        delete block;
        parent->SetChild(child,0 );
    }
    m_brick = parent; // if block
    //wxMessageDialog dlg2(NULL, _T("colse end"), _T("test"));
    //dlg2.ShowModal();
}
...
void CreateNassiSwitchChild::operator() ( wxChar const *first, wxChar const *last ) const
{
    ...
    child = brick->GetNext();
    brick->SetNext((NassiBrick *)NULL);
    brick->SetParent((NassiBrick *)NULL);
