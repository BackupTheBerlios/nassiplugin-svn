...
void NassiPlugin::OnAttach()
{
    ...
    for ( int i = 0 ; i < MaxInsertMenuEntries ; i++ )
        Connect(insertCFromDiagram[i], wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(NassiPlugin::OnInsertCFromDiagram), NULL, this);
    Connect(idParseC, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(NassiPlugin::ParseC), NULL, this);
}
...
void NassiPlugin::OnRelease(bool appShutDown)
{
    if ( !appShutDown )
    {
        ...
        for ( int i = 0 ; i < MaxInsertMenuEntries ; i++ )
            Disconnect(insertCFromDiagram[i], wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(NassiPlugin::OnInsertCFromDiagram), NULL, this);
        Disconnect(idParseC, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(NassiPlugin::ParseC), NULL, this);
    }
}
...
void NassiPlugin::BuildMenu(wxMenuBar* menuBar)
{
    ...
    {
        wxMenuItem *menuitm = filemenu->FindItem(pos);
        if (filenewmenu) filenewmenu = menuitm->GetSubMenu();
    }
    else
    {
...
void NassiPlugin::BuildModuleMenu(const ModuleType type, wxMenu* menu, const FileTreeData* data)
{
    ...
    if ( stc->GetLexer() == wxSCI_LEX_CPP && names.GetCount() > 0 )
    {
        ...
        for ( int i = 0; i < names.GetCount() && i < 10 ; i++ )
