...
NassiBrick *NassiBrick::SetParent(NassiBrick *par)
{
    ...
    parent = par;
    if ( parent )
        previous = (NassiBrick *)NULL;
    return( tmp );
}
NassiBrick *NassiBrick::SetChild(NassiBrick *brick, wxUint32 n)
{
    if ( brick != (NassiBrick *)NULL )
        delete brick;
    return((NassiBrick *)NULL);
}
...
NassiIfBrick::~NassiIfBrick()
{
    if ( TrueChild != (NassiBrick *)NULL )
        delete TrueChild;
    TrueChild = (NassiBrick *)NULL;
    if ( FalseChild != (NassiBrick *)NULL )
        delete FalseChild;
    FalseChild = (NassiBrick *)NULL;
}
...
NassiBrick *NassiIfBrick::SetChild(NassiBrick *brick, wxUint32 n)
{
    if ( brick )
    {
        ...
        brick->SetPrevious((NassiBrick *)NULL);
    }
    NassiBrick *tmp;
    if ( n == 0 )
    {
...
void NassiSwitchBrick::AddChild(wxUint32 pos = 0)
{
    ...
    //childBlocks.Insert((NassiBrick*) NULL, pos);
    wxUint32 p = 0;
    ArrayOfNassiBrickPtrs::iterator it = childBlocks.begin();
    std::vector<wxString *>::iterator cit = Comments.begin();
    std::vector<wxString *>::iterator sit = Sources.begin();
    ...
    childBlocks.insert(it, (NassiBrick*) NULL);
...
void NassiBricksCompositeIterator::First()
{
    ...
    child = 0;
    if ( itr != (NassiBricksCompositeIterator *)NULL )
        delete itr;
    itr = (NassiBricksCompositeIterator *)NULL;
}
