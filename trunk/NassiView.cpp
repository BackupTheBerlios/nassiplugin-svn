...
NassiView::NassiView(NassiFileContent *nfc):
    ...
    m_DrawComment(true),
    m_GraphBricks(),
    m_diagramwindow(NULL),
    m_graphFabric(NULL),
    m_updated(false),
    itsTask(NULL),
    hasSelectedBricks(false),
    reverseSelected(false),
    firstSelectedGBrick(NULL),
    lastSelectedGBrick(NULL),
    ChildIndicatorParent(NULL),
...
void NassiView::CopyBricks()
{
    ...
    NassiDataObject *ndo = NULL;
    if ( HasSelectedBricks() )
    {
        NassiBrick *first = (NassiBrick *)NULL,
                    *last = (NassiBrick *)NULL,
                     *tmp = (NassiBrick *)NULL;
    ...
    else
    {
        if ( ChildIndicatorIsSelected )
        {
            ...
            if ( parent )
                ndo = new NassiDataObject(NULL, this, *parent->GetTextByNumber(2+2*ChildIndicator), *parent->GetTextByNumber(3+2*ChildIndicator));
        }
    }
...
void NassiView::Paste()
{
    ...
    wxClipboardLocker locker;
    if ( !locker ) return;
    NassiDataObject  dataobj( (NassiBrick *)NULL, this);

    if ( wxTheClipboard->Open() )
    {
        if ( wxTheClipboard->IsSupported( wxDataFormat(NassiFormatId) ) )
        {
...
bool NassiView::CanPaste()const
{
    ...
    return wxTheClipboard->IsSupported(wxDataFormat(NassiFormatId));
}
...
HooverDrawlet *NassiView::OnMouseMove(wxMouseEvent &event, const wxPoint &pos)
{
    ...
    if ( !HasSelection() && gbrick && gbrick->IsOverText(pos) != NULL )
    {
        cursorOverText = true;
...
void NassiView::OnKeyDown(wxKeyEvent &event)
{
    ...
    if ( lastSelectedGBrick != NULL || event.ShiftDown() ) return;
    //ClearSelection();
...
void NassiView::DragStart()
{
    ...
    if ( HasSelectedBricks() )
    {
        ...
        dataptr = new NassiDataObject(NULL, this, strc, strs);
...
wxDragResult NassiView::OnDrop(const wxPoint &pos, NassiBrick *brick, wxString strc, wxString strs, wxDragResult def)
{
    ...
    wxCommand *delcmd = NULL, *addcmd = NULL;
...
HooverDrawlet *NassiView::OnDragOver(const wxPoint &pos, wxDragResult &def, bool HasNoBricks)
{
    ...
    if ( gbrick )
    {
        ...
        if ( drawlet == NULL ) def = wxDragError;
        return drawlet;
    }
